<template>
  <main class="min-h-screen bg-gray-100 flex flex-col">
    <header :class="[
      ...
      <div class="hidden md:block w-2/3 max-w-lg">
        <div class="relative">
      ...
      <div class="bg-white rounded-lg w-11/12 md:w-1/2 shadow-lg max-h-[80vh] flex flex-col animate-scale-in">
        <div class="flex items-center justify-between border-b px-4 py-3">
          ...
          <button @click="clearSearch"
            class="text-gray-500 hover:text-gray-700 p-1 rounded-full hover:bg-gray-100 transition-colors duration-200 focus:outline-none">&times;</button>
        </div>
        <div class="p-4 flex-1 overflow-y-auto">
          <ul v-if="filteredSearchResults.length">
            <li v-for="result in filteredSearchResults" :key="result.id"
              class="border-b border-gray-100 last:border-b-0 py-3 px-2 rounded-md hover:bg-gray-50 cursor-pointer transition-colors duration-200"
