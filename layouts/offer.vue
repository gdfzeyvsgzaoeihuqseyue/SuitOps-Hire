<template>
  <main class="min-h-screen bg-gray-100">
    ...
    <header :class="[
      ...
      <!-- Desktop Logo -->
      <div class="hidden lg:block" :class="isCollapsed ? 'max-w-full mr-4' : 'max-w-0 overflow-hidden'">
        <NuxtLink to="/"
          class="flex items-center justify-center hover:scale-105 hover:opacity-80 transition-transform duration-300">
      ...
      <section class="flex items-center gap-2 lg:gap-4 ml-auto">
        <template v-if="isAuthenticated">
          ...
          <button @click="$router.push('/dashboard/notification')" type="button"
            class="relative rounded-full text-gray-600 hover:text-primary p-2">
          ...
          <div class="relative">
            ...
            <div v-if="showProfileMenu"
              class="absolute right-0 mt-2 w-48 rounded-md bg-white shadow-lg border border-gray-200 z-50">
              <div class="border-b">
              ...
              <div class="border-b">
                ...
                <NuxtLink to="/dashboard/applications" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"
                  @click="closeProfileMenu">
                  Mes candidatures
                </NuxtLink>
              </div>
        ...
        <template v-else>
          <NuxtLink to="/auth/login"
            class="rounded-md bg-primary px-2 py-1.5 lg:px-3.5 lg:py-2.5 text-xs lg:text-sm font-semibold text-white shadow-sm hover:bg-secondary">
            Connexion
          </NuxtLink>
        </template>
      </section>
    ...
    <aside class="fixed top-0 left-0 h-screen bg-white shadow-lg transition-all duration-300 flex flex-col z-50" :class="[
      isMobileSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0',  // Mobile
      isCollapsed ? 'lg:w-20' : 'lg:w-64', // Desktop
      'w-64' // Largeur
    ]">
      ...
      <div class="flex items-center justify-between border-b px-4 py-3">
        ...
        <NuxtLink to="/"
          class="flex items-center justify-center hover:scale-105 hover:opacity-80 transition-transform duration-300"
          :class="isCollapsed ? 'lg:max-w-0 lg:overflow-hidden' : 'lg:max-w-full'">
          <!-- Desktop Logo -->
      ...
      <nav class="mt-5 px-2 space-y-1 flex-1 overflow-y-auto">
        <NuxtLink v-for="item in navItems" :key="item.path" :to="item.path" :title="item.label"
          class="group flex items-center p-2 text-sm rounded-lg transition-colors" :class="route.path === item.path
            ? 'bg-bgClr text-primary font-medium'
      ...
      <!-- Sidebar Footer -->
      <div v-show="!isCollapsed || isMobileSidebarOpen" class="border-t p-4 bg-gray-800 text-gray-400 space-y-4">
        <div class="text-sm mb-3 grid grid-cols-1 md:grid-cols-2 gap-2 border-b pb-4">
          <div v-for="link in footerLinks" :key="link.name">
            <a v-if="link.external" :href="link.path" class="hover:text-white text-xs" target="_blank"
              rel="noopener noreferrer">
              {{ link.label }}
            </a>
            <NuxtLink v-else :to="link.path" class="hover:text-white text-xs">
              {{ link.label }}
            </NuxtLink>
          </div>
        </div>
        ...
        <div class="flex justify-center">
          ...
          <div v-else class="flex items-center gap-3">
            <a v-for="item in socialLinks" :key="item.name" :href="item.href" class="text-sm leading-6 hover:text-white"
    ...
    <div class="pt-16 p-4 lg:p-8 transition-all duration-300" :class="[
      // Desktop only - no margin on mobile
